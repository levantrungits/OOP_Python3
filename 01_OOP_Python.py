...
class NLP(object):
    ...
    def __set_stopwords(self):
        self.stopwords = FileReader(setting.STOP_WORDS).read_stopwords()
...
class FeatureExtraction(object):
    ...
    def __build_dictionary(self):
        ...
        FileStore(filePath=setting.DICTIONARY_PATH).store_dictionary(dict_words)

    def __load_dictionary(self):
        if os.path.exists(setting.DICTIONARY_PATH) == False:
            self.__build_dictionary()
        self.dictionary = FileReader(setting.DICTIONARY_PATH).load_dictionary()
...
if __name__ == '__main__':
    json_train = DataLoader(dataPath=setting.DATA_TRAIN_PATH).get_json()
    FileStore(filePath=setting.DATA_TRAIN_JSON, data=json_train).store_json()
    json_test = DataLoader(dataPath=setting.DATA_TEST_PATH).get_json()
    FileStore(filePath=setting.DATA_TEST_JSON, data=json_test).store_json()
    train_loader = FileReader(filePath=setting.DATA_TRAIN_JSON)
    test_loader = FileReader(filePath=setting.DATA_TEST_JSON)
